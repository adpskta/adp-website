...
## 営業戦略（2026-09-03 坂田さん確認・07-07版を訂正）

**狙う仕事**
- **戸建て住宅の設計** ／ **住宅リノベーションの設計施工** が主軸（※07-07版で「マンションリノベ」に絞っていたのは狭すぎた。訂正済み）
- **店舗の設計施工**も本音では取りたい
- 共通項は「**設計施工で受けられる規模**」。期間あたり利益が良く、規模的に慣れていて問題が起きづらい

**エリア**
- 本来は**全国どこでも可**。ただし交通費・移動負荷を考え、**HP上の訴求は首都圏近郊に置いておく**（実際の相談は全国から受ける）

**フック事例**
- 「生前からある都市に住む」（renovation-of-new-wild・東京都・設計施工）。施主が書いたSUUMOジャーナル記事（https://suumo.jp/journal/2024/01/16/200044/）を事例ページにリンク済み。Selected Works先頭に配置
...
- 返信目安は「1週間以内」（Contactページに明記＝対外的な約束。応募・問い合わせの返信期限の根拠になる）
- **問い合わせフォームに電話番号欄は設けない**（2026-08-28 坂田さん決定）。項目を増やさず心理的ハードルを上げない方針
- **このリポジトリはGitHub public。個人情報と経営情報は置かない**
  - 応募者・問い合わせ者の個人情報 → `_a.d.p management/04 人事関連/` 側
  - **アクセス解析レポート・営業数値** → `_a.d.p management/_マネジメントサポート/10_HPレポート/` 側（`_reports/` は.gitignore済み）
  - ⚠️ 現状このCLAUDE.md自体にも広告費・営業戦略・旧サイト訪問数が書かれている。**次に整理する際は経営情報をマネジメント側へ移すことを検討**
- **HP経由の受信をマネジメントへ渡す手順は `_a.d.p management/_マネジメントサポート/09_HP連携フロー.md` が正**。種別ごとの行き先・概要ファイルの書式・受信箱への記載方法をそこに定義済み。HP経由の問い合わせや応募を扱うときは必ず参照する
